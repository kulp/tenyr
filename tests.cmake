...
foreach(run_path ${RUNS})
    ...
    check_std_outputs(
        NAME    "run ${run} icarus"
        COMMAND vvp
        ARGS    -M "${TENYR_LEGACY_BUILD_DIR}" -m vpidevices -N "${CMAKE_SOURCE_DIR}/tenyr" "+LOAD=${run}.texe" +DUMPENDSTATE
        INPUT   "${run}.texe"
    )
...
if (SDL)
    ...
    foreach(run_path ex/bm_mults.texe)
        ...
        check_std_outputs(
            NAME    "run ${run} icarus"
            COMMAND vvp
            ARGS    -M "${TENYR_LEGACY_BUILD_DIR}" -m vpidevices -N "${CMAKE_SOURCE_DIR}/tenyr" "+LOAD=${run_path}" +DUMPENDSTATE
            INPUT   "${run}.texe"
        )
...
foreach(op_path ${OPS})
    ...
    check_std_outputs(
        NAME    "op ${op} icarus"
        COMMAND vvp
        ARGS    -M "${TENYR_LEGACY_BUILD_DIR}" -m vpidevices -N "${CMAKE_SOURCE_DIR}/tenyr" "+LOAD=${op}.texe" +DUMPENDSTATE
        INPUT   "${op}.texe"
    )
...
check_std_outputs(
    ...
    COMMAND vvp
    ARGS    -M "${TENYR_LEGACY_BUILD_DIR}" -m vpidevices -N "${CMAKE_SOURCE_DIR}/tenyr" "+LOAD=nonexistent.texe"
    PROPERTIES
        WILL_FAIL TRUE
)
...
foreach(stem ${icarus_stems})
    ...
    check_std_outputs(
        ...
        COMMAND vvp
        ARGS    -M "${TENYR_LEGACY_BUILD_DIR}" -m vpidevices -N "${CMAKE_SOURCE_DIR}/tenyr" "+LOAD=${stem}.texe"
        INPUT   "${stem}.texe"
        PROPERTIES
            WILL_FAIL TRUE
    )
